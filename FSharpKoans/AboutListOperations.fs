    ...
    *)
    ...
    let ``05 Fixed-function mapping, the hard way (part 1).`` () =
        let map (xs : int list) : int list =
            ...
            let rec addi list1 list2 =
               match list2 with
               |[] -> List.rev list1
               |head :: tail -> addi ((head+1) :: list1) tail
            addi [] xs     
    ...
    let ``06 Fixed-function mapping, the hard way (part 2).`` () =
        let map (xs : int list) : int list =
            ...
            let rec doubl list1 list2 =
               match list2 with
               |[] -> List.rev list1
               |head :: tail -> doubl ((head*2) :: list1) tail
            doubl [] xs 
   ...
   *)
    ...
    let ``07 Specified-function mapping, the hard way`` () =
        let map (f : 'a -> 'b) (xs : 'a list) : 'b list =
            ...
            let rec addi list1 list2 =
               match list2 with
               |[] -> List.rev list1
               |head :: tail -> addi (f(head) :: list1) tail
            addi [] xs 
    ...
    let ``09 Specified-function filtering, the hard way`` () =
        let filter (f : 'a -> bool) (xs : 'a list) : 'a list =
            ...
            let rec fil list1 list2 =
               match list2 with
               |[] -> List.rev list1
               |head :: tail -> match f(head) with 
                                |true -> fil ((head) :: list1) tail
                                |false -> fil list1 tail              
            fil [] xs              
    ...
    let ``11 Fixed-function filtering, the hard way`` () =
        let filter (xs : int list) : int list =
            ...
            let rec oddn list1 list2 =
               match list2 with
               |[] -> List.rev list1
               |head :: tail -> match ((head%2) <> 0) with 
                                |true -> oddn ((head) :: list1) tail
                                |false -> oddn list1 tail              
            oddn [] xs   
...
*)
    ...
    let ``14 A fold which sums a list`` () =
        let fold initialState xs =
            ...
            let rec fol staten list2 =
               match list2 with
               |[] -> staten
               |head :: tail -> fol (staten+head) tail             
            fol initialState xs   
    ...
    let ``15 A fold which multiplies a list`` () =
        let fold initialState xs =
            ...
            let rec mult staten list2 =
               match list2 with
               |[] -> staten
               |head :: tail -> mult (staten*head) tail             
            mult initialState xs 
    ...
    let ``16 Folding, the hard way`` () =
        let fold (f : 'a -> 'b -> 'a) (initialState : 'a) (xs : 'b list) : 'a =
            ...
            let rec foldn staten list2 =
               match list2 with
               |[] -> staten
               |head :: tail -> foldn (f staten head) tail             
            foldn initialState xs 
        fold (+) 0 [1;2;3;4] |> should equal 10
