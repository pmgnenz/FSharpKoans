...
*)
   ...
   *)
    ...
    let ``08 An identifier pattern will match anything`` () =
        ...
        let y = "dream" // replace with a string
        let z = 67.0 // replace with anything else!
        x |> should be ofType<int>
        y |> should be ofType<string>

    [<Test>]
    let ``09 A wildcard pattern will match anything`` () =
        let _ = 40 // replace with an integer
        let _ = "flare" // replace with a string
        let _ = 1050 // replace with anything else!
        ()
